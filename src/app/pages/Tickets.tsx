    ...
    <>
      ...
      <div className="flex-1 overflow-auto bg-gray-50 p-8">
        <div className="flex justify-between items-center mb-6">
          <div className="text-[13px] text-gray-500">{tickets.length} total tickets</div>
          <button
        ...
        <div className="grid grid-cols-4 gap-4">
          {columns.map(col => {
            ...
            return (
              <div
                key={col.key}
                className={`bg-gray-100 rounded-lg p-3.5 border-t-2 ${col.color} border border-gray-200 min-h-[400px] transition-all ${
                  isDragOver ? 'border-gray-400 bg-gray-50 shadow-sm' : ''
                }`}
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="text-[12px] text-gray-700" style={{ fontWeight: 600 }}>{col.label}</span>
                  <span className="text-[11px] text-gray-500 bg-white px-2 py-0.5 rounded border border-gray-200 tabular-nums" style={{ fontWeight: 500 }}>{colTickets.length}</span>
                </div>
                <div className="space-y-2.5">
                  {colTickets.map(ticket => (
                    <button
                      ...
                      className="bg-white rounded-md p-3.5 border border-gray-200 cursor-pointer active:cursor-grabbing hover:border-gray-400 hover:shadow-sm transition-all group w-full text-left"
                      aria-label={`${ticket.title} - ${ticket.priority} priority`}
                    >
                      <div className="flex items-start gap-2">
                        <GripVertical className="w-3.5 h-3.5 text-gray-300 group-hover:text-gray-500 mt-0.5 shrink-0 transition-colors" strokeWidth={1.5} />
                        <div className="flex-1 min-w-0">
                          <div className="text-[12px] text-gray-900 group-hover:text-gray-900 mb-2 transition-colors" style={{ fontWeight: 500 }}>{ticket.title}</div>
                          <div className="text-[10px] text-gray-400 mb-3">{ticket.unit} &middot; {ticket.createdDate}</div>
                  ...
                  {colTickets.length === 0 && (
                    <div className="text-center py-8 text-[12px] text-gray-400">No tickets</div>
                  )}
                </div>
              </div>
            );
      ...
      {!isEditing ? (
        <div className="space-y-4">
          ...
          <div className="pb-4 border-b border-gray-200">
            <h3 className="text-[15px] text-gray-900 mb-3" style={{ fontWeight: 600 }}>{ticket.title}</h3>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[11px] text-gray-500 uppercase tracking-wider mb-1.5" style={{ fontWeight: 500 }}>Unit</label>
              <div className="text-[13px] text-gray-900">{ticket.unit}</div>
            </div>
            <div>
              <label className="block text-[11px] text-gray-500 uppercase tracking-wider mb-1.5" style={{ fontWeight: 500 }}>Assignee</label>
              <div className="text-[13px] text-gray-900">{ticket.assignee}</div>
            </div>
            <div>
              <label className="block text-[11px] text-gray-500 uppercase tracking-wider mb-1.5" style={{ fontWeight: 500 }}>Created Date</label>
              <div className="text-[13px] text-gray-900">{ticket.createdDate}</div>
            </div>
          </div>

          <div>
            <label className="block text-[11px] text-gray-500 uppercase tracking-wider mb-1.5" style={{ fontWeight: 500 }}>Description</label>
            <div className="text-[13px] text-gray-700 whitespace-pre-wrap">{ticket.description || 'No description provided'}</div>
          </div>

          <div className="flex justify-end gap-3 pt-2">
            <button onClick={onClose} className="px-4 py-2 rounded-md text-[12px] text-gray-600 border border-gray-200 hover:border-gray-400 hover:text-gray-900 hover:bg-gray-50 transition-all" style={{ fontWeight: 500 }}>Close</button>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button onClick={() => setIsEditing(false)} className="px-4 py-2 rounded-md text-[12px] text-gray-600 border border-gray-200 hover:border-gray-400 hover:text-gray-900 hover:bg-gray-50 transition-all" style={{ fontWeight: 500 }}>Cancel</button>
